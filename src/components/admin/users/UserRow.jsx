...
const UserRow = ({ user, handleSubscriptionUpdate, handleUserUpdate }) => {
	...
	const handle_VIRTUVE_CANCELED = () => {
		handleUserUpdate(user.id, 'subscription_type', 'Canceled_virtuve');
	};
			...
			<UserBox>
				...
				<SideBox>
					...
					<div className={styles.lastActivity}>
						Prisijungė:
						{user.last_activity ? (
							<span>&nbsp;&nbsp;{date_with_time(user.last_activity)}</span>
						) : (
							<span>&nbsp;&nbsp;Nežinoma</span>
						)}
					</div>
			...
			<UserBox>
				<SideBoxRow>
					<FaRegCopy
						...
						onClick={() => {
							navigator.clipboard.writeText(user.facebook_name);
						}}
					/>
					<SideBox>
			...
			<UserBox>
				...
				<SideBox>
					<span className={styles.sideBoxTitle}>
						{user.plan_assign_status === 'week'
							? '2 savaitės'
							: user.plan_assign_status === 'month'
								? '1 mėnesis'
								: 'Statusas'}
					</span>
