...
import { Check, ArrowRight, Star, BadgePercent } from 'lucide-react';
        ...
        <div className={styles.priceCard} style={{top: `calc(${responsiveNavHeight} + 1rem)`}}>
            <div className={styles.priceCardContent}>
                ...
                <div className={styles.trustSignals}>
                    {trustSignals.map(signal => <div key={signal} className={styles.signal}>
                    ...
                    <div className={styles.ratingStars}>
                        {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                                key={star}
                                size={18}
                                fill='var(--light-green-500)' color='var(--light-green-500)'
                            />
                        ))}
                    </div>
                    <p className={styles.reviewStats}>
                    4.9/5 · Pagal 2500+ klientų atsiliepimus
                    </p>
                </div>
            </div>
        </div>
