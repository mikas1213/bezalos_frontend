				...
				<Grid space="clamp(2rem, 4vw, 6rem)" className={styles.heroContainer} min="50px">
					<Stack className={`${styles.left} ${isMounted ? styles.onload : ''}`}>
						...
						<Box className={styles.subTitle}>
							<h2>
								Čia išmoksi sveikatai palankios mitybos pagrindų, tapsi bendruomenės nare. Juk drauge įpročius
								formuoti lengviau!
							</h2>
						</Box>

						<Box className={styles.btnContainer}>
							<button type="button" className={styles.btn} onClick={() => navigate('/virtuve')}>
								Virtuvė
							</button>
						</Box>
					</Stack>
